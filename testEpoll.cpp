...
int main(int argc, char *argv[]) {
    ...
    if (listen(listen_fd, SOMAXCONN) == -1) {
        ...
        std::cout << "start server at port [" << port << "] with [" << (argc <= 1 ? "LT" : "ET") << "] mode." <<
                std::endl;
        ...
        for (int i = 0; i < n; ++i) {
            if (epoll_events[i].events & EPOLLIN) //trigger read event
            {
                if (epoll_events[i].data.fd == listen_fd) {
                    ...
                    std::cout << "EPOLLIN event triggered for client fd [" << epoll_events[i].data.fd << "]." <<
                            std::endl;
                    ...
                    std::cout << "recv data from client fd [" << epoll_events[i].data.fd << "] and data is [" << recvbuf
                            << "]." << std::endl;
                }
